...
export function assertStatistics(employees, dtoOut) {
	...
	assertTruncAge(minAge, dtoOut.minAge, 'minAge');
	assertTruncAge(maxAge, dtoOut.maxAge, 'maxAge');


	let medianAge = median(ages);
	assertTruncAge(medianAge, dtoOut.medianAge, 'medianAge');
...
export function assertTruncAge(expected, actual, name) {
	const min = (expected - DATE_TOLERACE) | 0;
    const max = (expected + DATE_TOLERACE) | 0;
	if(YEARS_ROUNDED) {
		assert(+(expected.toFixed(0)) === actual, 'Testing statistics - '+name+' '+actual+' should be '+(expected.toFixed(0)) + '. See ReadMe - this variable should be rounded to 0 decimal places.');
	} else {
		assert(Number.isInteger(actual), 'Testing statistics - '+name+' '+actual+' should be an integer. See ReadMe - this variable should be truncated on output.');
		assert(min <= actual, 'Testing statistics - '+name+' '+actual+' should be '+(expected | 0) + '. See ReadMe - this variable should be truncated on output.');
		assert(actual <= max, 'Testing statistics - '+name+' '+actual+' should be '+(expected | 0) + '. See ReadMe - this variable should be truncated on output.');
	}
}
